...
async def send_main_menu(upsert: UpsertFn, message, chat_id: int | None, user_id: int):
    ...
    builder.row(CallbackButton(text="💰 Финансовая поддержка", payload="fin_support"))
...
async def send_contacts(upsert: UpsertFn, message, chat_id: int | None, user_id: int):
    builder = InlineKeyboardBuilder()
    builder.row(LinkButton(text="Мой бизнес", url="https://мойбизнес-24.рф"))
    builder.row(CallbackButton(text="◀️ В главное меню", payload="back_main"))
    await upsert(message, chat_id, user_id, text=CONTACTS_MAIN_TEXT, attachments=[builder.as_markup()])
...
async def send_fin_garant(upsert: UpsertFn, message, chat_id: int | None, user_id: int, *, text: str):
    builder = InlineKeyboardBuilder()
    builder.row(
        LinkButton(
            text="Гарантийная поддержка",
            url="https://xn---24-9cdulgg0aog6b.xn--p1ai/sections/RGO/",
        )
    )
    builder.row(CallbackButton(text="⬅️ Вернуться назад", payload="back_fin_org"))
    await upsert(message, chat_id, user_id, text=text, attachments=[builder.as_markup()])
